...
| [FabricOps MCP-Driven Data Architecture](https://github.com/WilsonH918/fabric-custom-remote-mcp) | Python, Azure Functions, Model Context Protocol (MCP), Microsoft Fabric REST API, Power BI | A custom remote MCP server hosted on Azure Functions that acts as a data architecture integration layer for AI agents. It exposes MCP tools defined in `server.py` to enable secure orchestration of Microsoft Fabric and Power BI operations—such as workspace creation, report analysis, DAX execution, and automated fixes—via REST APIs and enterprise-grade governance. |
